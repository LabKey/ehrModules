...
var helper = org.labkey.ldk.query.LookupValidationHelper.create(LABKEY.Security.currentContainer.id, LABKEY.Security.currentUser.id, 'ehr_compliancedb', 'employeelocations');
...
function afterUpdate(row, oldRow, errors){
    ...
    for (var i=0;i<fields.length;i++){
        ...
        if (row[fieldName] && oldRow[fieldName] && row[fieldName] != oldRow[fieldName]){
            helper.cascadeUpdate('ehr_compliancedb', 'employees', 'location', row[fieldName], oldRow[fieldName]);
        }
    }
}
