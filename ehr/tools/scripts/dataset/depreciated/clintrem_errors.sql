 ...
 */
SELECT id, FixDateTime(date, time) AS Date, (pno) AS pno, (userid) AS userid, FixNewlines(remark) AS remark, FixNewlines(remark) AS description, ts, uuid AS objectid, null as parentid
FROM clintrem
WHERE id IS NULL OR id = '' and pno NOT REGEXP '^[0-9]+$'
