 ...
 */
SELECT id, FixDate(date) AS Date, (sex) AS sex, (weight) AS weight, (wdate) AS wdate, Timestamp(Date('1970-01-01'), wtime)  AS wtime, (dam) AS dam, (sire) AS sire, (room) AS room, (cage) AS cage, (cond) AS cond, (origin) AS origin, (conception) AS conception, (type) AS type, FixNewlines(remark) AS remark, ( CONCAT_WS(', ',      CASE WHEN sex IS NULL  OR sex=''  THEN NULL ELSE CONCAT('sex: ', sex)  END, 
