 ...
 */
SELECT lower(id) as id, FixDate(date) AS Date, (account) AS account, (source) AS source, (result) AS result, FixNewlines(remark) AS remark, (antibiotic) AS antibiotic, (sensitivity) AS sensitivity,
( CONCAT_WS(',\n ',
     ...
     ) ) AS Description, b.ts, b.uuid AS objectid, NULL as parentid
FROM bacteriology b
