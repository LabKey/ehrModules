...
SELECT lower(id) as id, FixDate(date) AS Date, (seq1) AS seq1, (seq2) AS seq2, (b.code) AS code,
    (SELECT group_concat(DISTINCT uuid) as uuid FROM (SELECT * FROM biopsyhead GROUP BY id, date, caseno, account) b2 WHERE b.id = b2.id AND b.date = b2.date limit 1) AS parentid,
      ( CONCAT_WS(',\n',
