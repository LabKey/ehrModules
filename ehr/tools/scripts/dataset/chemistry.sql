...
SELECT id, FixDate(date) AS Date, (account) AS account, ( CONCAT_WS(', ', 
     ...
     AS Description, (glucose) AS glucose, (bun) AS bun, (creatinine) AS creatinine, (ck_cpk) AS ck_cpk, (uricacid) AS uricacid, (cholesterol) AS cholesterol, (triglyc) AS triglyc, (sgot_ast) AS sgot_ast, (ldh) AS ldh, (tbili) AS tbili, (ggt) AS ggt, (sgpt_alt) AS sgpt_alt, (tprotein) AS tprotein, (albumin) AS albumin, (phosphatase) AS phosphatase, (calcium) AS calcium, (phosphorus) AS phosphorus, (iron) AS iron, (sodium) AS sodium, (potassium) AS potassium, (chloride) AS chloride, (remark) AS remark, (clinremark) AS clinremark FROM chemistry
     WHERE id != ''
