...
ts, uuid AS objectid,
(select UUID from clinhead t2 WHERE t1.id=t2.id AND t1.date=t2.date AND t1.time=t2.time AND (t1.pno=t2.pno OR t1.pno is null) GROUP BY t1.uuid limit 1) as parentid,
